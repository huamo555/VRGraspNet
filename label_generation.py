...
def process_grasp_labels(end_points):
    ...
    seed_xyzs = end_points['xyz_graspable']  # 从 end_points 字典中获取可抓取点的坐标 seed_xyz_graspable (B, M, 3)
    ...
    batch_grasp_scores = []
    batch_grasp_widths = []
    for i in range(batch_size): # 对于每个批次中的样本，逐个处理可抓取点和物体姿态
        seed_xyz = seed_xyzs[i]  # (Ns, 3)
        # 对于每个样本，获取物体的姿态 poses，形状为 [(3, 4),]，其中 (3, 4) 表示物体的旋转矩阵和平移向量
        ...
        for obj_idx, pose in enumerate(poses):
            ...
            # generate and transform template grasp views 对于每个物体姿态，将抓取点和抓取视角进行变换
