...
def evaluate(dump_dir):
    ...
    save_dir = os.path.join(cfgs.dump_dir, 'ap_{}_Seen.npy'.format(cfgs.camera))
    np.save(save_dir, res)

    ge = GraspNetEval(root=cfgs.dataset_root, camera=cfgs.camera, split='test_similar')
    res, ap = ge.eval_similar(dump_folder=dump_dir, proc=6)
    save_dir = os.path.join(cfgs.dump_dir, 'ap_{}_Similar.npy'.format(cfgs.camera))
    np.save(save_dir, res)
    ...
    save_dir = os.path.join(cfgs.dump_dir, 'ap_{}_Novel.npy'.format(cfgs.camera))
    np.save(save_dir, res)
